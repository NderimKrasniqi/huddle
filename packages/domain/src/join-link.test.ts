...
describe('JOIN_LINK_SCHEME', () => {
  ...
  it.each(['tv', 'phone'])('is the scheme the %s app registers', (app) => {
    expect(registeredScheme(app)).toBe(JOIN_LINK_SCHEME);
  });
});

describe('the Phone route a Join Link opens', () => {
  ...
  // matching the link's path against a file in `apps/phone/app`. Nothing
  ...
  it('is a file expo-router will resolve the link to', () => {
    ...
    expect(
      existsSync(path.join(appsDir, 'phone', 'app', ...routeDir, '[code].tsx')),
    ).toBe(true);
  });
});
