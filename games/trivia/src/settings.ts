...
// `questions` is the persisted key from the launch-proof module. Keep it
// stable while the rest of Trivia's settings grow around it.
const QUESTION_COUNT_KEY = 'questions';
...
export function triviaSettings(chosen: GameSettings | undefined): TriviaSettings {
  ...
  const categories = [EVERY_CATEGORY, ...PACK_CATEGORIES] as const;
  return {
    scoring: oneOf(SCORING_MODES, settled[SCORING_KEY], DEFAULT_SCORING),
    questions: questionCount(settled[QUESTION_COUNT_KEY]),
    ...
    category: oneOf(categories, settled[CATEGORY_KEY], EVERY_CATEGORY),
  };
}
...
export const TRIVIA_SETTINGS_PRESENTATION = {
  ...
  customSettingKeys: [SCORING_KEY, QUESTION_COUNT_KEY, DIFFICULTY_KEY, QUESTION_SECONDS_KEY, CATEGORY_KEY],
} as const;
