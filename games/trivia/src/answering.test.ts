...
import { triviaGameLogic, type PlayableTriviaState, type TriviaState } from './logic';
...
function gameWith(...playerIds: readonly string[]): PlayableTriviaState {
  const initial = triviaGameLogic.createInitialState({
...
describe('a phone with no question to answer', () => {
  it('keeps the intro on a shared eyes-up instruction', () => {
    const initial = triviaGameLogic.createInitialState({
      players: [player(ADA), player(GRACE)],
      settings: {},
    });

    expect(answerScreen(initial, ADA)).toEqual({
      kind: 'eyesUp',
      line: 'Get ready — eyes up on the TV.',
    });
  });
