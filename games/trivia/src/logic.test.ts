import { type GamePlayer, type GameSettings, settingsFrom } from '@huddle/domain';
...
function asPlayable(state: TriviaState): PlayableTriviaState {
  if (state.phase === 'entered') {
    throw new Error('expected a playable Trivia state');
  }

  return state;
}

function createdOn(chosen: GameSettings | undefined, ...playerIds: readonly string[]): TriviaState {
...
function advanceIntro(state: TriviaState): PlayableTriviaState {
  if (state.phase !== 'intro') {
    throw new Error('expected Trivia intro');
  }
...
function gameWith(...playerIds: readonly string[]): PlayableTriviaState {
  return startedOn(undefined, ...playerIds);
}

/** A game started on the Host's choices, settled the way `startGame` settles them. */
function startedOn(chosen: GameSettings | undefined, ...playerIds: readonly string[]): PlayableTriviaState {
  return advanceIntro(createdOn(chosen, ...playerIds));
}
 ...
 */
function shortGameWith(...playerIds: readonly string[]): PlayableTriviaState {
...
function questionOf(state: PlayableTriviaState) {
...
function rightAnswerTo(state: PlayableTriviaState): number {
  return questionOf(state).correctIndex;
}

function wrongAnswerTo(state: PlayableTriviaState): number {
...
function advancing(state: PlayableTriviaState): PlayableTriviaState {
  return asPlayable(triviaGameLogic.reduce(state, advanceOf(state)));
}

/** The deadline event addressed to the beat this state is on. */
function advanceOf(state: PlayableTriviaState) {
  return {
    kind: 'advance',
    questionIndex: state.questionIndex,
    phase: state.phase,
...
function scoreOf(state: PlayableTriviaState, playerId: string): number | undefined {
...
describe('the questions a game is dealt', () => {
  ...
  it('is exactly as many questions as the Host chose', () => {
    expect(startedOn({ questions: '5' }, ADA, GRACE).questions).toHaveLength(5);
    expect(startedOn({ questions: '20' }, ADA, GRACE).questions).toHaveLength(20);
  });

  it('is only the category the Host chose', () => {
    const movies = startedOn({ category: 'Movies', questions: '20' }, ADA, GRACE);
  ...
  it('can be filtered to any category the pack offers, and never comes up empty', () => {
    for (const category of PACK_CATEGORIES) {
      const filtered = startedOn({ category, questions: '5' }, ADA, GRACE);
  ...
  it('spans the pack when the Host chose every category', () => {
    ...
    const dealt = startedOn({ category: EVERY_CATEGORY, questions: '5' }, ADA, GRACE);

    expect(new Set(dealt.questions.map(categoryOf)).size).toBe(5);
  });

  it('never asks the same question twice in one game', () => {
    const dealt = startedOn({ questions: '20' }, ADA, GRACE);
...
describe('a game of trivia', () => {
  it('starts in a server-owned intro and keeps the legacy v2 state readable', () => {
    const initial = createdOn(undefined, ADA, GRACE);

    expect(initial.phase).toBe('intro');
    if (initial.phase !== 'intro') throw new Error('expected Trivia intro');
    expect(initial.questions).toHaveLength(10);
    expect(introTimer(initial)).toMatchObject({ afterMs: INTRO_SECONDS * 1000 });
    expect(advanceIntro(initial).phase).toBe('question');

    const legacy = {
      phase: 'entered' as const,
      resolvedSettings: { questions: 5 as const },
    };
    expect(triviaGameLogic.stateVersion).toBe(2);
    expect(triviaStateSchema.parse(legacy)).toEqual(legacy);
    expect(triviaGameLogic.reduce(legacy, {
      kind: 'answer',
      playerId: ADA,
      questionIndex: 0,
      optionIndex: 0,
  ...
  it('ignores a second “move on” for a beat the room has left', () => {
    ...
    const second = asPlayable(triviaGameLogic.reduce(asPlayable(triviaGameLogic.reduce(revealed, endReveal)), endReveal));
...
describe('a game played to the end', () => {
   ...
   */
  function playthrough(): PlayableTriviaState {
...
describe('the clock a question runs on', () => {
  ...
  function timerOn(state: PlayableTriviaState) {
  ...
  it('uses the selected ten- and thirty-second timers', () => {
    const ten = startedOn({ questionSeconds: '10' }, ADA, GRACE);
    const thirty = startedOn({ questionSeconds: '30' }, ADA, GRACE);

    expect(questionTimer(ten)?.afterMs).toBe(10_000);
    expect(questionTimer(thirty)?.afterMs).toBe(30_000);
  });
  ...
  it('is addressed to the question on screen, so the room actually moves on', () => {
    ...
    const expired = asPlayable(triviaGameLogic.reduce(answered, timerOn(asked).event));
...
describe('the Scoring Mode a Host chose', () => {
  ...
  function gameScored(scoring: string): PlayableTriviaState {
  ...
  it('prices speed answers against the selected ten- and thirty-second timer', () => {
    const ten = startedOn({ scoring: 'speed', questionSeconds: '10' }, ADA, GRACE);
    const thirty = startedOn({ scoring: 'speed', questionSeconds: '30' }, ADA, GRACE);

    const tenReveal = answering(
      answering(ten, ADA, rightAnswerTo(ten), 5_000),
      GRACE,
      wrongAnswerTo(ten),
    );
    const thirtyReveal = answering(
      answering(thirty, ADA, rightAnswerTo(thirty), 15_000),
      GRACE,
      wrongAnswerTo(thirty),
    );

    expect(scoreOf(tenReveal, ADA)).toBe(150);
    expect(scoreOf(thirtyReveal, ADA)).toBe(150);
  });
  ...
  it('scores a game dealt before speed scoring flat, rather than on nothing', () => {
    ...
    const dealtEarlier: PlayableTriviaState = { ...asked, scoring: undefined, answerSeconds: undefined };
    const revealed = answering(
...
describe('the game as a client is shown it', () => {
  const redact = (state: PlayableTriviaState, viewer: string | undefined): PlayableTriviaState =>
    asPlayable(triviaGameLogic.redactStateFor?.(state, viewer) ?? state);

  it('removes every other player id while the question is up', () => {
  ...
  it('shows a player their own answer', () => {
    ...
    expect(redact(answered, GRACE).participationCount).toBe(1);
  });

  it('gives every viewer only an aggregate participation count', () => {
    const answered = answering(gameWith(ADA, GRACE), GRACE, 2);

    const shown = redact(answered, undefined);
    expect(shown.answers).toEqual({});
    expect(shown.participationCount).toBe(answersIn(answered));
    expect(shown.participationCount).toBe(1);

    const phone = redact(answered, ADA);
    expect(phone.answers).toEqual({});
    expect(phone.participationCount).toBe(1);
  });
  ...
  it('gives the TV the shared prompt and normalized outcomes at reveal', () => {
    const playing = gameWith(ADA, GRACE);
    ...
    expect(revealed.phase).toBe('reveal');
    // The reveal is what the answer key was being held back for. The TV gets
    // only the current shared prompt and a normalized per-player verdict map.
  ...
  it('keeps reveal details off every phone, including the answer owner', () => {
    ...
    for (const viewer of [ADA, GRACE]) {
      const shown = redact(revealed, viewer);
  ...
  it('keeps answer keys, choices, and timings out of the finished wire', () => {
    let state = shortGameWith(ADA, GRACE);
    ...
    for (const viewer of [undefined, ADA]) {
      const shown = redact(state, viewer);

      expect(shown.questions.every((question) => question.correctIndex === HIDDEN_CORRECT_INDEX)).toBe(true);
      expect(shown.answers).toEqual({});
      expect(shown.answerSeconds).toBeUndefined();
      expect(shown.revealVerdicts).toBeUndefined();
      expect(shown.standings).toEqual(state.standings);
    }
  });
  ...
  it('survives a state that carries no answers at all', () => {
    ...
    const playing = { ...gameWith(ADA, GRACE), answers: undefined } as unknown as PlayableTriviaState;
