...
import { settingsFrom } from '@huddle/domain';
...
export function PickAGameScreen({
  ...
  readonly setupDraft:
    | { readonly gameId: string; readonly settings: GameSettings; readonly mode: GameSetupMode; readonly stage: 'configuring' | 'ready'; readonly readyPlayerIds: readonly string[] }
    | null
    | undefined;
  ...
  function chooseMode(nextMode: GameSetupMode) {
    if (!selected || setupDraft?.stage === 'ready') return;
    setMode(nextMode);
  ...
  async function reopenSetup() {
    ...
    if (sessionToken === null) return;
    try {
      await reopenGameSetup({ sessionToken });
    } catch (error) {
      setSelectionFailure(lifecycleFailureMessage(error));
    }
  }
...
function GameSettingsScreen({
  ...
  readonly settingsChoice: SettingsChoice | undefined;
  readonly setupDraft: { readonly stage: 'configuring' | 'ready'; readonly readyPlayerIds: readonly string[] };
  readonly roster: readonly RosterSeat[];
  ...
  return (
    <PhoneScreen contentStyle={styles.settingsScreenContent}>
      ...
      <ModeTabs mode={mode} onChoose={onChooseMode} disabled={setupDraft.stage === 'ready'} />

      {setupDraft.stage === 'ready' ? (
        <View style={styles.field}>
          <PresetSummary schema={schema} settings={settings} />
          <StatusStrip variant="success">Setup locked — everyone can Ready now.</StatusStrip>
          <PrimaryButton label="Edit setup" enabled onPress={onReopenSetup} />
        </View>
      ) : mode === 'custom' ? (
        <SettingsControls
          schema={schema}
...
function GameCardChip({ icon, label }: { readonly icon: IconName; readonly label: string }) {
  return (
    <View style={styles.gameCardChip}>
      <View style={[styles.absoluteFill, styles.gameCardChipWash]} />
...
function StartGameControl({
  ...
  readonly setupDraft: { readonly stage: 'configuring' | 'ready'; readonly readyPlayerIds: readonly string[] } | null | undefined;
}) {
  ...
  async function start() {
    ...
    try {
      ...
      if (setupDraft?.stage !== 'ready') {
        await finalizeGameSetup({ sessionToken });
        await setGameReady({ sessionToken, ready: true });
      } else if (host !== undefined && !setupDraft.readyPlayerIds.includes(host.playerId)) {
        await setGameReady({ sessionToken, ready: true });
      } else {
  ...
  const host = roster.find((seat) => seat.host);
  const hostReady = host !== undefined && setupDraft?.readyPlayerIds.includes(host.playerId) === true;
  const everyoneReady = setupDraft?.stage === 'ready' && roster.every((seat) => !seat.away && setupDraft.readyPlayerIds.includes(seat.playerId));
  const label = setupDraft?.stage !== 'ready'
    ? `Lock setup & Ready`
    : !hostReady
      ? `I’m ready`
      : everyoneReady
  ...
  const pressable = selected && control.enabled && !starting && (setupDraft?.stage !== 'ready' || !hostReady || everyoneReady);
  ...
  return (
    <View style={styles.field}>
      ...
      {setupDraft?.stage === 'ready' && hostReady && !everyoneReady ? (
        <Text style={styles.waitingFor}>{setupDraft.readyPlayerIds.length} of {roster.length} players ready.</Text>
      ) : null}
