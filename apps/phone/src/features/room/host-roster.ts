...
/**
 * The Host's roster: one row per player, what each row's right-hand slot says,
 * and the count line under them.
 *
 * It is the only surface in Huddle that says a *non-Host* player is away
 * between games, and that product requirement is why it exists.
 ...
 *
 * `away` was the state the approved Phone board did not have — it is the status dot's own
 ...
 * an earlier board drew it as a pink NEW! pill, and the approved board brings
 * it back in the system's one informational blue. How
...
/** What each slot reads as aloud; the approved board calls the green dot “online.” */
...
/**
 * The line under the rows: "<n> players in — you can start anytime".
 *
 * The invitation is dropped when it is false: the start control directly below
