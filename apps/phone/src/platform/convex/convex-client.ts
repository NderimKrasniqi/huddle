...
/**
 * The Phone's connection to Convex, created once per launch and handed to
 * `ConvexProvider` in the root layout.
 *
 * The deployment URL is configuration, never a literal: Metro inlines
 * `EXPO_PUBLIC_CONVEX_URL` at bundle time from `apps/phone/.env` (the
 * committed default: the huddle cloud dev deployment, the same one the TV app
 * points at) or `apps/phone/.env.local` (yours, gitignored).
 */
...
if (deploymentUrl === undefined || deploymentUrl === '') {
  ...
  throw new Error(
    'EXPO_PUBLIC_CONVEX_URL is not set — the Phone has no Convex deployment to join a room on. See apps/phone/.env.',
  );
}
