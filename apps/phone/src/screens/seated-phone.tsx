...
import { type Arrivals, isGreeting, JUST_JOINED_MS, noteArrivals } from '@huddle/domain';
...
import { styles } from './seated-phone-styles';
...
export function SeatedPhone({
  ...
  // to reset state when an input changes, and the pattern `PhoneScreen` already
  // uses for its notice.
...
function WaitingScreen({
  ...
  readonly playerId: RosterSeat['playerId'];
  ...
  const ready = setupDraft?.readyPlayerIds.includes(playerId) === true;
  ...
  async function changeReady() {
    ...
    try {
      await setGameReady({ sessionToken, ready: !ready });
    } finally {
  ...
  return (
    <PhoneScreen>
      ...
      {setupDraft?.stage === 'ready' ? (
        <PrimaryButton
          label={ready ? 'Ready — tap to undo' : 'I’m ready'}
          enabled={!changingReady}
          loading={changingReady}
          onPress={() => void changeReady()}
        />
      ) : null}
    </PhoneScreen>
  );
  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={onDismiss}>
      <View style={styles.sheetRoot}>
        ...
        <Pressable
          style={[styles.absoluteFill, styles.sheetScrim]}
          onPress={onDismiss}
...
/**
 * The Host's controls for one player over the lobby: make them Host or remove
 * them from the room.
 *
...
/**
 * The Host's tap that starts the game from the lobby.
 *
