...
import { cardEntryOffset, carouselFooterLine } from '../features/carousel';
import { closeExpiredRoom, deployed, keepRoomPresent, openRoom } from '../platform/room-session';
import {
  keepOpeningRoom,
  type OpenRoom,
  type RoomOpening,
...
import { TvStage } from '../features/game-session';
...
function OpenRoomStage({
  ...
  if (screen.kind === 'game') {
    return (
      <GameStage
        ...
        state={screen.state}
        clockRemainingMs={screen.clockRemainingMs}
        roster={roster ?? []}
      />
    );
  }
  ...
  if (screen.kind === 'paused' || screen.kind === 'unavailable') {
    return <TvRuntimeStatus kind={screen.kind} />;
  }
  ...
  //
  // Soft Minimal switched here on `seats.length > 0`, which meant the Room Code
 ...
 * between games.
 *
 * One screen rather than two is the whole change from Soft Minimal here. That
...
function TvRuntimeStatus({ kind }: { readonly kind: 'paused' | 'unavailable' }) {
  return (
    <TvStage>
 ...
 * just joined.
 *
 * It spent a release somewhere else. Soft Minimal's television had no seat with a
