...
| UI-001 | The Phone Join Room route matches the illustrated code/QR surface with normalization, responsive keyboard-safe layout, and accessibility states; every other active Phone, TV, and module screen remains a centered `PurposeScreen` label | Join Room and purpose render tests; purpose mapping tests; UI-stack/architecture validator |
| UI-002 | Shared tokens, app configuration, and packaging assets stay white/black neutral; the only runtime artwork is the three exact Phone-specific Join Room PNGs | token tests; app configuration validator; asset hash/dimension validator |
...
| REL-003 | Phone Join Room screenshots cover standard/compact and keyboard-open/closed layouts; remaining Phone/TV screens preserve the centered baseline, with physical camera and remote gates tracked separately | Join Room visual inspection; [clean-slate validation](design/qa/evidence/platform-consolidation/2026-08-14-clean-slate-validation.md); render tests; dated device checklist |
