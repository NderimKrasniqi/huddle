...
/**
 * `soft-minimal/tokens-only` exercised through ESLint itself, against the repo's
...
const RULE = 'soft-minimal/tokens-only';
...
describe('a design value written at a call site', () => {
  ...
  // way in any case: the rule fires on Soft Minimal's *own* ink, so what it is
  ...
  it('is caught as a shadow depth on the surface that takes one', async () => {
    ...
    // theme happens to hold. It was Soft Minimal's `punch` until the Soft Minimal
...
describe('a design value taken from the theme', () => {
  ...
  // one: reading `ink` a second time would otherwise report Soft Minimal's own
  ...
  // design value rather than one of Soft Minimal's, and the palette holds no token
...
describe('what the rule leaves alone', () => {
  ...
  // read on Soft Minimal's one shadow-bearing surface and nowhere else, which is
