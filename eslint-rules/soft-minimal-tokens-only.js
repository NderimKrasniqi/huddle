...
/**
 * `soft-minimal/tokens-only` — a Soft Minimal design value is never written at a call
 ...
 * `style-objects.js`, shared with `soft-minimal/body-text-floor` — which has to
 ...
 * than one of Soft Minimal's: `0` for a radius, a border width or a shadow depth —
...
/** Soft Minimal's one shadow-bearing surface, and the props of it that carry a design value. */
...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        'Soft Minimal colours, radii, border widths, shadow depths and font families come from a packages/ui token, never from a value written at the call site.',
    },
    schema: [],
    messages: {
      literal:
        '{{property}} is Soft Minimal\'s to decide: take it from `{{token}}` in @huddle/ui rather than writing {{literal}} here.',
      declaration:
        '{{property}} is Soft Minimal\'s to decide: `{{name}}` is declared in this file and is not built from the theme, so take it from `{{token}}` in @huddle/ui instead.',
      imported:
        '{{property}} is Soft Minimal\'s to decide: `{{name}}` comes from {{source}} rather than from the theme, so take it from `{{token}}` in @huddle/ui instead.',
      fontWeight:
