...
/**
 * `boardwalk/body-text-floor` — body text is never set below the smallest size
 ...
 * added under it.
 *
 * ## Why this is not an option on `boardwalk/tokens-only`
 *
...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        "Body text is never set below the smallest size Boardwalk allows on the surface it is read from (`minBodyFontSize` in packages/ui).",
    },
    schema: [
      {
        type: 'object',
  ...
  create(context) {
    ...
    if (floor === undefined) {
      throw new Error(
        `boardwalk/body-text-floor: no floor for surface '${surface}' in ${JSON.stringify(floors)}.`,
      );
    }
